...
export default function OrderDetailsModal({ order, isOpen, onClose, userDisplayName }) {
  if (!isOpen || !order) return null;

  const customerName =
    userDisplayName ||
    order.user_name ||
    [order.user_first_name, order.user_last_name].filter(Boolean).join(' ').trim() ||
    '-';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                    ...
                    <p className="text-sm">
                      <span className="font-medium">Mijoz:</span> {customerName}
                    </p>
                    <p className="text-sm">
                      <span className="font-medium">Bosqich:</span> {formatOrderStage(order.current_stage)}
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                    ...
                    <p className="text-sm">
                      ...
                      <span className="font-semibold">{(order.total_amount || 0).toLocaleString()} so&apos;m</span>
                    </p>
                  </div>
                </div>
              </div>
