...
import { toE164Uzbek, isValidUzLocalPhone } from '../utils/phone';
...
export default function Login() {
  ...
  const normalizePhone = (phone) => toE164Uzbek(phone);

  const setPhone = (formattedLocal) => {
    if (isSetupMode) {
      setSetupForm((prev) => ({ ...prev, phone: formattedLocal }));
    } else {
      setLoginForm((prev) => ({ ...prev, phone: formattedLocal }));
    }
    setError('');
    setSuccess('');
  };
  ...
  const handleSendCode = async (e) => {
    e.preventDefault();
    if (!isValidUzLocalPhone(setupForm.phone)) {
      setError('Telefon raqamini to‘liq kiriting (masalan: 90 123 45 67)');
      return;
    }
  ...
  const handleLoginSubmit = async (e) => {
    ...
    if (!isValidUzLocalPhone(loginForm.phone)) {
      setError('Telefon raqamini to‘liq kiriting (masalan: 90 123 45 67)');
      setLoading(false);
      return;
    }
    ...
    try {
      const response = await apiLogin(normalizePhone(loginForm.phone), loginForm.password.trim());
        ...
        <form onSubmit={isSetupMode ? (setupStep === 1 ? handleSendCode : setupStep === 2 ? handleVerifyCode : handleSetPassword) : handleLoginSubmit} className="space-y-6">
          ...
          <div>
            ...
            <PhoneInput
              ...
              onChange={setPhone}
              disabled={loading || (isSetupMode && setupStep > 1)}
            />
          </div>
