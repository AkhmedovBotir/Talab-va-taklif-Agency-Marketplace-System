...
import { buildMarketplaceUserDetailFields } from '../utils/entityDetails';
...
export default function MarketplaceUsers() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        setError('');
        const response = await getMarketplaceUsers(filters);
        setRows(response.data || []);
      } catch (err) {
        setError(err.message || 'Marketplace foydalanuvchilarini yuklashda xatolik');
        setRows([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <Layout>
      <div>
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Marketplace foydalanuvchilari</h1>
        ...
        <DetailViewModal
          title={
            selectedItem
              ? `Foydalanuvchi: ${formatMarketplaceUserName(selectedItem) || '—'}`
              : 'Foydalanuvchi'
          }
          isOpen={Boolean(selectedItem)}
          onClose={() => setSelectedItem(null)}
          fields={selectedItem ? buildMarketplaceUserDetailFields(selectedItem) : []}
        />
                    ...
                    rows.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 text-sm font-medium text-gray-900">
                          {formatMarketplaceUserName(item) || '-'}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-700">{item.first_name || '-'}</td>
                        <td className="px-6 py-4 text-sm text-gray-700">{item.last_name || '-'}</td>
                        <td className="px-6 py-4 text-sm">
                          <ViewDetailsButton onClick={() => setSelectedItem(item)} />
                        </td>
                      </tr>
                    ))
                  )}
