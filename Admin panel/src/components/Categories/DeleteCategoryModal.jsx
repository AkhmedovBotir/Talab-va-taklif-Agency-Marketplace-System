...
const DeleteCategoryModal = ({ open, onClose, onSuccess, item, isSubcategory = false }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = isSubcategory ? await subcategoryAPI.delete(id) : await categoryAPI.delete(id);
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b">
                <h2 className="text-lg font-bold text-gray-800">{isSubcategory ? 'Subkategoriyani o`chirish' : 'Kategoriyani o`chirish'}</h2>
                ...
                <div className="flex gap-4 mb-6">
                  ...
                  <div>
                    <p className="text-gray-700 mb-2">
                      <span className="font-semibold">{item.name}</span> ni o`chirishni tasdiqlaysizmi?
                    </p>
