...
const HorizontalBarChart = ({ data, labelKey, valueKey, maxValue, color = 'bg-indigo-500', formatValue }) => {
  ...
  const max = maxValue || Math.max(...data.map((item) => Number(item?.[valueKey]) || 0));
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between text-sm gap-2">
              <span className="font-medium text-gray-700 truncate max-w-[220px]" title={label || '-'}>
                {label || '-'}
              </span>
            ...
            <div className="h-6 bg-gray-100 rounded-full overflow-hidden">
              ...
              >
                {percentage > 15 && <span className="text-xs text-white font-medium">{percentage.toFixed(0)}%</span>}
              </motion.div>
            </div>
          </motion.div>
        );
