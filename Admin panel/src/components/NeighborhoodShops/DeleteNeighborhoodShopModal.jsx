...
const DeleteNeighborhoodShopModal = ({ open, onClose, onSuccess, row }) => {
  const { showSuccess, showError } = useSnackbar();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const id = row?.id ?? row?._id;
  const name = row?.name || 'Nomaʼlum';
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const res = await neighborhoodShopAPI.delete(id);
          ...
          >
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b">
                <h2 className="text-lg font-bold text-gray-800">Do‘konni o‘chirish</h2>
              ...
              <div className="p-6">
                ...
                <div className="flex gap-4 mb-6">
                  ...
                  <div>
                    <p className="text-gray-700 mb-2">Bu maxalla do‘konini o‘chirishni tasdiqlaysizmi?</p>
...
export default DeleteNeighborhoodShopModal;
