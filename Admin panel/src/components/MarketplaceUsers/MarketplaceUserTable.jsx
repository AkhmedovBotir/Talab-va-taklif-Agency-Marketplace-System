...
const MarketplaceUserTable = ({
  ...
  const handleStatusToggle = async (row, status) => {
    ...
    try {
      const res = await marketplaceUserAPI.updateStatus(id, status);
      if (res.success) {
  ...
  if (!rows.length) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <p className="text-center text-gray-500">Marketplace foydalanuvchilar topilmadi</p>
      </div>
    );
  }
        ...
        <table className="w-full min-w-[1100px]">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ism</th>
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Familiya</th>
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Telefon</th>
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Jinsi</th>
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Viloyat</th>
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tuman</th>
              <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase">MFY</th>
            ...
            {rows.map((row, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-sm font-medium text-gray-900">{row.first_name || '-'}</td>
                  <td className="px-3 py-3 text-sm text-gray-700">{row.last_name || '-'}</td>
                  <td className="px-3 py-3 text-sm text-gray-700">{row.phone || '-'}</td>
                  <td className="px-3 py-3 text-sm text-gray-700">{row.gender || '-'}</td>
                  <td className="px-3 py-3 text-sm text-gray-700">{row.region?.name || lookupName(regionMap, row.region_id) || '-'}</td>
                  <td className="px-3 py-3 text-sm text-gray-700">{row.district?.name || lookupName(districtMap, row.district_id) || '-'}</td>
                  <td className="px-3 py-3 text-sm text-gray-700">{row.mfy?.name || lookupName(mfyMap, row.mfy_id) || '-'}</td>
