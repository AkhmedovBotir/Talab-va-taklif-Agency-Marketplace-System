...
const DeleteMarketplaceUserModal = ({ open, onClose, onSuccess, row }) => {
  const { showSuccess, showError } = useSnackbar();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const id = row?.id ?? row?._id;
  const name = `${row?.first_name || ''} ${row?.last_name || ''}`.trim() || "Noma'lum";
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const res = await marketplaceUserAPI.delete(id);
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b">
                <h2 className="text-lg font-bold text-gray-800">Foydalanuvchini o'chirish</h2>
              ...
              <div className="p-6">
                ...
                <div className="flex gap-4 mb-6">
                  ...
                  <div>
                    <p className="text-gray-700 mb-2">Bu foydalanuvchini o'chirishni tasdiqlaysizmi?</p>
                    <div className="bg-gray-50 p-3 rounded text-sm">
                      <p className="font-medium text-gray-900">{name}</p>
                      {row?.phone && <p className="text-gray-500">Tel: {row.phone}</p>}
                    </div>
...
export default DeleteMarketplaceUserModal;
