...
import GeoCascadeSearchableFields from './GeoCascadeSearchableFields';
import ContragentTypeSearchableSelect from './ContragentTypeSearchableSelect';

const EditDistrictContragentModal = ({ open, onClose, onSuccess, contragentId, regions, districts, mfys }) => {
    ...
    (async () => {
      ...
      try {
        ...
        setForm({
          ...
          logo,
          activity_type_id:
            d.activity_type_id != null
              ? String(d.activity_type_id)
              : String(d.activity_type?.id ?? d.contragent_type?.id ?? ''),
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/^\d{9}$|^\d{12}$/.test(inn)) {
      setError('INN faqat 9 yoki 12 ta raqam bo‘lishi kerak');
      return;
    }
    ...
    if (!form.region_id || !form.district_id || !form.mfy_id || !form.activity_type_id) {
      setError('Viloyat, tuman, MFY va faoliyat turini tanlang');
      return;
    }
    ...
    try {
      ...
      const res = await contragentAPI.update(contragentId, payload);
      if (res.success) {
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-6 border-b">
                <h2 className="text-xl font-bold text-gray-800">Kontragentni tahrirlash</h2>
              ...
              {fetching ? (
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <ContragentTypeSearchableSelect
                      required
                      value={form.activity_type_id}
                      onChange={(e) => setForm((p) => ({ ...p, activity_type_id: e.target.value }))}
                      disabled={loading}
                    />
                  </div>
...
export default EditDistrictContragentModal;
