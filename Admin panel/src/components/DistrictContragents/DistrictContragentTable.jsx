...
const activityTypeId = (row) =>
  row.activity_type_id ?? row.activityType?.id ?? row.activity_type?.id ?? row.contragent_type_id;

const DistrictContragentTable = ({
  rows,
  loading,
  onEdit,
  onDelete,
  ...
  const typeMap = useMemo(() => buildNameMap(contragentTypes), [contragentTypes]);

  const regionLabel = (row) =>
    row.region?.name || row.region_name || lookupName(regionMap, row.region_id) || '-';
  const districtLabel = (row) =>
    row.district?.name || row.district_name || lookupName(districtMap, row.district_id) || '-';
  const mfyLabel = (row) => row.mfy?.name || row.mfy_name || lookupName(mfyMap, row.mfy_id) || '-';
  const activityLabel = (row) =>
    row.activity_type?.name ||
    row.contragent_type?.name ||
    row.activityType?.name ||
    lookupName(typeMap, activityTypeId(row)) ||
    '-';
  ...
  const handleStatusToggle = async (row, status) => {
    ...
    try {
      const res = await contragentAPI.updateStatus(id, status);
      if (res.success) {
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">INN</th>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Faoliyat</th>
            ...
            {rows.map((row, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">{row.inn || '-'}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700 max-w-[140px] truncate" title={activityLabel(row)}>
                    {activityLabel(row)}
                  </td>
...
export default DistrictContragentTable;
