...
const TABS = [
  { id: 'reviews', label: 'Barcha Baholar', icon: RateReview, component: Reviews },
  { id: 'contacts', label: 'Salbiy Aloqalar', icon: ContactMail, component: Contacts },
];

const BASE_PATH = '/dashboard/reviews';
...
const ReviewsMain = () => {
  ...
  const defaultTab = allowedTabs[0]?.id || 'reviews';
  ...
  const setActiveTab = (tabId) => {
    if (allowedTabIds.length === 0 || allowedTabIds.includes(tabId)) {
      navigate(`/dashboard/reviews/${tabId}`);
    }
  };
