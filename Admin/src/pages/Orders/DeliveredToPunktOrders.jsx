...
const DeliveredToPunktOrders = ({ hideHeader = false }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await adminDataAPI.getOrdersDeliveredToPunkt(params);
      ...
      const errorMsg = err.message || 'Punktga yuborilgan buyurtmalarni yuklashda xatolik yuz berdi';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      {!hideHeader && (
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Punktga Yuborilgan Buyurtmalar</h1>
            <p className="text-gray-600">Kontragentlar tomonidan punktga yetkazilgan buyurtmalar</p>
          </div>
        </motion.div>
      )}
...
export default DeliveredToPunktOrders;
