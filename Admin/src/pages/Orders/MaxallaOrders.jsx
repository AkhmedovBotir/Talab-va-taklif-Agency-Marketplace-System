...
import { maxallaOrdersAPI, adminDataAPI } from '../../services/api';
...
const MaxallaOrders = ({ activeTab, activeTabConfig, hideHeader = false }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const apiMethod = maxallaOrdersAPI[activeTabConfig?.apiMethod];
      if (!apiMethod) {
  ...
  return (
    <div>
      ...
      {!hideHeader && (
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Maxalla Buyurtmalari</h1>
            <p className="text-gray-600">Maxalla do'konlari buyurtmalarini ko'rish va monitoring qilish</p>
          </div>
        </motion.div>
      )}
