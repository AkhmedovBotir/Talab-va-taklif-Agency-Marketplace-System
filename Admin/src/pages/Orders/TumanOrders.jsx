...
import { tumanOrdersAPI, adminDataAPI } from '../../services/api';
...
const TumanOrders = ({ activeTab, activeTabConfig, hideHeader = false }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const apiMethod = tumanOrdersAPI[activeTabConfig?.apiMethod];
      if (!apiMethod) {
  ...
  return (
    <div>
      ...
      {!hideHeader && (
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Tuman Buyurtmalari</h1>
            <p className="text-gray-600">Tuman kontragentlari buyurtmalarini ko'rish va monitoring qilish</p>
          </div>
        </motion.div>
      )}
