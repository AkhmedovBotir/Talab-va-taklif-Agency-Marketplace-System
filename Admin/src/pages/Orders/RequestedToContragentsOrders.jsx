...
const RequestedToContragentsOrders = ({ hideHeader = false }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await adminDataAPI.getOrdersRequestedToContragents(params);
      ...
      const errorMsg = err.message || 'Kontragentlarga yuborilgan buyurtmalarni yuklashda xatolik yuz berdi';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      {!hideHeader && (
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Kontragentlarga Yuborilgan Buyurtmalar</h1>
            <p className="text-gray-600">Kontragentlarga so'rov yuborilgan buyurtmalar</p>
          </div>
        </motion.div>
      )}
...
export default RequestedToContragentsOrders;
