...
const ConfirmedByCustomersOrders = ({ hideHeader = false }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await adminDataAPI.getOrdersConfirmedByCustomers(params);
      ...
      const errorMsg = err.message || 'Foydalanuvchi qabul qilgan buyurtmalarni yuklashda xatolik yuz berdi';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      {!hideHeader && (
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Foydalanuvchi Qabul Qilgan Buyurtmalar</h1>
            <p className="text-gray-600">Mijozlar tomonidan qabul qilingan va tasdiqlangan buyurtmalar</p>
          </div>
        </motion.div>
      )}
...
export default ConfirmedByCustomersOrders;
