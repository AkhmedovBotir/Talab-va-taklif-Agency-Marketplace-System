...
const BASE_PATH = '/dashboard/contragents';
...
const Contragents = ({ hideHeader = false }) => {
  ...
  const defaultTab = allowedTabs[0]?.id || 'types';
  const activeTab = allowedTabIds.length === 0
    ? (['types', 'tuman', 'mfy'].includes(tabParam) ? tabParam : defaultTab)
    : (allowedTabIds.includes(tabParam) ? tabParam : defaultTab);

  useEffect(() => {
    if (allowedTabIds.length > 0 && tabParam && !allowedTabIds.includes(tabParam)) {
      navigate(`/dashboard/contragents/${defaultTab}`, { replace: true });
    }
  }, [allowedTabIds, tabParam, defaultTab, navigate]);

  const setActiveTab = (tabId) => {
    if (allowedTabIds.length === 0 || allowedTabIds.includes(tabId)) {
      navigate(`/dashboard/contragents/${tabId}`);
    }
  };
