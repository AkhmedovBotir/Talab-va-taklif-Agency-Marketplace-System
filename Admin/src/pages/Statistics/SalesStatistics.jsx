...
const BASE_PATH = '/dashboard/statistics';
...
const SalesStatistics = () => {
  ...
  const defaultTab = allowedTabs[0]?.id || 'summary';
  ...
  const setActiveTab = (tabId) => {
    if (allowedTabIds.length === 0 || allowedTabIds.includes(tabId)) {
      setNavigationPath([]);
      navigate(`/dashboard/statistics/${tabId}`);
    }
  };
      ...
      >
        ...
        {activeTab === 'viloyats' && navigationPath.length === 2 && navigationPath[1]?.type === 'tuman' && (
          <SalesMfysTab
            ...
            onMfyClick={(mfy) => {
              // MFY daily breakdown – extend here if modal/detail view is needed
            }}
          />
        )}
      </motion.div>
