...
const EditContragentTypeModal = ({ open, onClose, onSuccess, contragentType }) => {
  ...
  useEffect(() => {
    if (open && contragentType) {
      setFormData({
        ...
        icon: contragentType.icon || '',
        status: contragentType.status || 'active',
      });
      setError('');
    }
  }, [open, contragentType]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await contragentTypeAPI.updateContragentType(contragentType._id, payload);

      if (response.success) {
        showSuccess(response.message || 'Kontragent turi muvaffaqiyatli yangilandi');
        onSuccess();
        onClose();
      }
    } catch (err) {
      const errorMsg = err.message || 'Kontragent turi yangilashda xatolik yuz berdi';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
              ...
              <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 text-white p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-bold">Kontragent Turini Tahrirlash</h2>
                      <p className="text-xs text-indigo-100 mt-0.5">
                        Kontragent turi ma'lumotlarini yangilang
                      </p>
                    </div>
                  </div>
                  <button
...
export default EditContragentTypeModal;
