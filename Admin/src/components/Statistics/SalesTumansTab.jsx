...
// Detail Modal
const TumanDetailModal = ({ tumanId, tumanName, filters, open, onClose, onMfyClick }) => {
                ...
                {data?.tuman && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="font-medium text-gray-900">
                      {data.tuman.parent?.name || ''} → {data.tuman.name} ({data.tuman.code})
                    </p>
                  </div>
                )}
                  ...
                  <div className="overflow-x-auto border border-gray-200 rounded-lg max-h-96">
                    <table className="w-full">
                      <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                        <tr>
                          <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">MFY</th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {data.data.map((item, index) => (
                          <tr key={item.mfy?._id || index} className="hover:bg-gray-50">
                            <td className="px-4 py-2 text-sm font-medium text-gray-900">
                              {item.mfy?.name || '-'}
                              {item.mfy?.code && <span className="text-xs text-gray-500 ml-2">({item.mfy.code})</span>}
                            </td>
...
const SalesTumansTab = ({ viloyatId: propViloyatId, onTumanClick }) => {
  const { showError } = useSnackbar();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (filters.viloyatId) {
        // Get tumans for specific viloyat
        response = await salesStatsAPI.getViloyatStats(filters.viloyatId, params);
      } else {
        // Get all tumans
        response = await salesStatsAPI.getByTumans({ ...params, viloyatId: filters.viloyatId });
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <TumanDetailModal
        ...
        onMfyClick={(mfy) => {
          // Navigate to mfys tab with selected mfy
          console.log('Navigate to mfy:', mfy);
        }}
      />
    </div>
  );
