...
const EditCategoryModal = ({ open, onClose, onSuccess, item, isSubcategory = false, categories = [] }) => {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      parent_id: String(item.parent_id ?? item.parent?.id ?? item.parent?._id ?? ''),
      password: '',
    });
    setFileName('');
  }, [open, item]);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const res = isSubcategory ? await subcategoryAPI.update(id, payload) : await categoryAPI.update(id, payload);
      if (res.success) {
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(ev) => ev.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b">
                <h2 className="text-xl font-bold text-gray-800">{isSubcategory ? 'Subkategoriyani tahrirlash' : 'Kategoriyani tahrirlash'}</h2>
                ...
                <div>
                  ...
                  {form.image && (
                    <div className="mt-2 flex items-center gap-2">
                      <button type="button" onClick={() => setShowImage((s) => !s)} className="text-sm text-indigo-600 inline-flex items-center gap-1">
                        {showImage ? <VisibilityOff className="w-4 h-4" /> : <Visibility className="w-4 h-4" />}
                        {showImage ? 'Yashirish' : "Ko'rish"}
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                  ...
                  {showImage && form.image && <img src={form.image} alt="preview" className="mt-2 max-h-32 rounded border border-gray-200" />}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
