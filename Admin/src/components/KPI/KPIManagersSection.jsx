...
import KPIManagerDetailModal from './KPIManagerDetailModal';
...
const KPIManagersSection = () => {
  ...
  const fetchManagers = async ({ page, limit } = {}) => {
    ...
    try {
      ...
      showError(error.message || 'Menejerlarni yuklashda xatolik');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">Menejerlar topilmadi</div>
      ) : (
        <>
                ...
                {managers.map((item, index) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="font-medium text-gray-900">{item.manager?.name || '-'}</div>
                      <div className="text-xs text-gray-500">{item.manager?.phone || '-'}</div>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {item.manager?.viloyat?.name || '-'}
                    </td>
...
export default KPIManagersSection;
