...
const KPITumanAgentsSection = () => {
  ...
  const fetchAgents = async ({ page, limit } = {}) => {
    ...
    try {
      ...
      const response = await kpiAPI.getTumanAgentsKPI(params);
      if (response.success) {
      ...
      showError(error.message || 'Tuman agentlarini yuklashda xatolik');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">Tuman agentlari topilmadi</div>
      ) : (
        <>
...
export default KPITumanAgentsSection;
