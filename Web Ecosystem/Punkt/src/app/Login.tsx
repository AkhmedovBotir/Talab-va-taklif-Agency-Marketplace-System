...
import { IoLocationOutline, IoLockClosed, IoEyeOff, IoEye, IoClose, IoLockOpenOutline } from 'react-icons/io5';
...
export function Login() {
  ...
  const requestDeviceCode = async () => {
    ...
    try {
      ...
      setError(ax.response?.data?.message || 'Kod yuborishda xatolik');
    } finally {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await login({ phone: fullPhone(phone), password }, deviceInfo || getDeviceInfo());
      navigate('/orders', { replace: true });
    } catch (err: unknown) {
      const ax = err as { response?: { data?: { message?: string; requiresDeviceVerification?: boolean }; status?: number } };
      const msg = ax.response?.data?.message || (err instanceof Error ? err.message : '');
      const status = ax.response?.status ?? 0;
      const requiresDevice = ax.response?.data?.requiresDeviceVerification === true || /qurilma|device|tasdiqlash/i.test(msg);
  ...
  const handlePasswordSetupStep1 = async () => {
    ...
    try {
      ...
      } else setError((res as { message?: string }).message || 'Xatolik');
    } catch (err: unknown) {
      const ax = err as { response?: { data?: { message?: string } } };
  ...
  const handlePasswordSetupStep2 = async (code?: string) => {
    ...
    try {
      ...
      else setError((res as { message?: string }).message || 'Kod noto\'g\'ri');
    } catch (err: unknown) {
      const ax = err as { response?: { data?: { message?: string } } };
      setError(ax.response?.data?.message || 'Kod noto\'g\'ri');
      setSmsCode(['', '', '', '', '']);
  ...
  const handlePasswordSetupStep3 = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        await login({ phone: fullPhone(setupPhone), password: setupPassword }, dev);
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.content}>
        <header className={styles.header}>
          <div className={styles.iconWrap}>
            <IoLocationOutline size={64} color="#007AFF" />
          </div>
          <h1 className={styles.title}>Punkt Dasturi</h1>
        ...
        <form className={styles.card} onSubmit={handleLogin}>
          ...
          {deviceVerificationRequired && (
            <div className={styles.deviceWarning}>
              ...
              <button type="button" className={styles.deviceWarningBtn} onClick={() => { setShowDeviceModal(true); setDeviceStep('request'); requestDeviceCode(); }}>
      ...
      {showDeviceModal && (
        <div className={styles.modalOverlay} onClick={() => !deviceLoading && (setShowDeviceModal(false), setDeviceCode(['', '', '', '', '']))}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h2 className={styles.modalTitle}>Qurilmani tasdiqlash</h2>
              <button type="button" className={styles.modalClose} onClick={() => !deviceLoading && setShowDeviceModal(false)} aria-label="Yopish">
                <IoClose size={24} />
              </button>
            </div>
            {deviceStep === 'verify' && (
              <div className={styles.modalBody}>
                <p className={styles.modalDesc}>{fullPhone(phone)} raqamiga yuborilgan 5 xonali kodni kiriting</p>
            ...
            {deviceStep === 'request' && deviceLoading && <p className={styles.modalLoading}>Kod yuborilmoqda...</p>}
          </div>
        </div>
      )}
            ...
            <div className={styles.modalBody}>
              ...
              {setupStep === 'password' && (
                <>
                  ...
                  <div className={styles.inputWrap}>
                    <IoLockClosed size={20} color="#666" className={styles.inputIcon} />
                    <input className={styles.input} type="password" placeholder="Takrorlang" value={setupPasswordConfirm} onChange={(e) => setSetupPasswordConfirm(e.target.value)} />
                  </div>
                  {error && <p className={styles.error}>{error}</p>}
