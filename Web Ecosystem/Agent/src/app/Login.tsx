...
export function Login() {
  ...
  const requestDeviceCode = async () => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('qurilma topilmadi') || msg.toLowerCase().includes('device not found')) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((status === 403 || status === 400) && requiresDevice) {
        ...
        const data = (ax.response?.data as { data?: { deviceId?: string } })?.data;
        if (data?.deviceId && deviceInfo) {
          setDeviceInfo({ ...deviceInfo, deviceId: data.deviceId });
        }
  ...
  const handlePasswordSetupStep1 = async () => {
    ...
    setError('');
    try {
      const res = await apiService.passwordSetupStep1(fullPhone(setupPhone));
      if (res.success) {
        setSetupStep('sms');
  ...
  const handlePasswordSetupStep2 = async (code?: string) => {
    ...
    setError('');
    try {
      const res = await apiService.passwordSetupStep2(fullPhone(setupPhone), c);
      if (res.success) {
  ...
  const handlePasswordSetupStep3 = async () => {
    ...
    setError('');
    try {
      const res = await apiService.passwordSetupStep3(fullPhone(setupPhone), setupPassword);
      if (res.success) {
    ...
    <div className={styles.wrap}>
      <div className={styles.content}>
        <header className={styles.header}>
          ...
          <h1 className={styles.title}>Agent Tizimi</h1>
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            {deviceStep === 'request' && (
              <div className={styles.modalBody}>
                <p className={styles.modalDesc}>SMS orqali yuboriladigan tasdiqlash kodini so‘raymiz.</p>
                {deviceLoading ? <p className={styles.modalLoading}>Kod yuborilmoqda...</p> : null}
              </div>
            )}
            {deviceStep === 'verify' && (
              <div className={styles.modalBody}>
                ...
                <button
                  ...
                  disabled={deviceLoading || deviceCode.some((d) => !d)}
      ...
      {/* Password setup (yangi foydalanuvchi) modal */}
      {showPasswordSetup && (
