...
const REGIONS_BASE_URL = 'http://localhost:5000/api';
const REVIEWS_BASE_URL = 'http://localhost:5000/api/reviews';
...
export interface Order {
  ...
  /** Maxalla: yetkazuvchi yetkazganini belgilagan */
  deliveredByProvider?: boolean;
...
/** Maxalla buyurtma to'lov holati (GET .../maxalla-orders/:id/payment-status) */
export interface MaxallaPaymentStatusResponse {
  success: boolean;
  data?: {
    orderId: string;
    orderNumber: string;
    totalPrice: number;
    paymentStatus: 'awaiting_delivery' | 'unpaid' | 'paid';
    totalPaid?: number;
    transactions?: any[];
    deliveredByProvider?: boolean;
  };
...
/** Maxalla buyurtma to'lov (POST .../maxalla-orders/:id/pay) */
export interface MaxallaPayResponse {
  success: boolean;
  message?: string;
  data?: { orderId: string; transactionsCreated?: number };
}
...
class ApiService {
      ...
      if (response.status === 401) {
        ...
        try {
          ...
          if (marketplaceKeys.length > 0) {
            await storage.multiRemove(marketplaceKeys);
          }
          
          // Emit event to notify AuthContext about 401 error
          appEvents.emit('marketplace:401-unauthorized');
        } catch (storageError) {
  ...
  // Get Maxalla Stores for Product (endpoint bir xil: /maxalla-products/:id/stores)
  async getMaxallaStores(
    ...
    if (!response.ok) {
      throw new Error(raw.message || 'Xatolik yuz berdi');
    }

    // Backend data yoki stores qaytarishi mumkin; bitta formatga keltiramiz
    const list = Array.isArray(raw.data)
      ? raw.data
      : Array.isArray((raw as any).stores)
        ? (raw as any).stores
        : [];
    const result: MaxallaStoresResponse = {
      success: raw.success !== false,
      count: raw.count ?? list.length,
      data: list,
    };
    console.log('getMaxallaStores dokonlar:', result);
    return result;
  }
  ...
  /**
   * Mijoz yetkazilganini tasdiqlash (shu bilan to'lov hisobga olinadi).
   * Tuman:  POST /api/marketplace/orders/:id/confirm-delivery
   * Maxalla: POST /api/marketplace/maxalla-orders/:id/confirm-delivery
   */
  ...
  /**
   * Maxalla: to'lov holati. GET /api/marketplace/maxalla-orders/:id/payment-status
   */
  async getMaxallaOrderPaymentStatus(
    orderId: string,
    token: string
  ): Promise<MaxallaPaymentStatusResponse> {
    return this.request<MaxallaPaymentStatusResponse>(
      `/maxalla-orders/${orderId}/payment-status`,
      { method: 'GET' },
      token
    );
  }
  ...
  /**
   * Maxalla: to'lov qilish. POST /api/marketplace/maxalla-orders/:id/pay
   */
  async payMaxallaOrder(
    orderId: string,
    token: string
  ): Promise<MaxallaPayResponse> {
    return this.request<MaxallaPayResponse>(
      `/maxalla-orders/${orderId}/pay`,
      { method: 'POST' },
      token
    );
  }
  ...
  // Payment: to'lov confirm-delivery orqali hisobga olinadi (POST .../confirm-delivery).
  // Quyidagi pay/payment-status endpointlari backendda olib tashlangan bo'lishi mumkin.
