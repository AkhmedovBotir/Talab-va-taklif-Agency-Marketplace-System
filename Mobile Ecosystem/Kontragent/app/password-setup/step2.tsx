...
export default function PasswordSetupStep2Screen() {
  ...
  const handleResendCode = async () => {
    setResending(true);
    try {
      await apiService.passwordSetupStep1({ phone });
      Alert.alert('Muvaffaqiyatli', 'Yangi tasdiqlash kodi telefon raqamingizga yuborildi');
    } catch (error: any) {
  ...
  const handleVerifyCode = async (codeValue?: string) => {
    ...
    if (codeString.length !== 5) {
      Alert.alert('Xatolik', 'Tasdiqlash kodi 5 ta raqamdan iborat bo\'lishi kerak');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Muvaffaqiyatli', 'Kod muvaffaqiyatli tasdiqlandi. Endi parol o\'rnatishingiz mumkin', [
        {
          text: 'OK',
          onPress: () => {
            router.push({
