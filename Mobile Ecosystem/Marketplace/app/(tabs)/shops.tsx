...
export default function CategoriesScreen() {
    ...
    const [categoryHasProducts, setCategoryHasProducts] = useState<Record<string, boolean>>({});
    const [subcategoryHasProducts, setSubcategoryHasProducts] = useState<Record<string, boolean>>({});
    ...
    useEffect(() => {
        ...
        Promise.all(
            categories.map(async (cat) => {
                try {
                    const res = await apiService.getProducts({ category: cat._id, limit: 1, status: 'active' });
                    return { id: cat._id, has: (res.data?.length ?? 0) > 0 };
                } catch {
                    return { id: cat._id, has: false };
                }
            })
        ).then((results) => {
            if (cancelled) return;
            results.forEach((r) => { map[r.id] = r.has; });
            setCategoryHasProducts(map);
        });
    ...
    useEffect(() => {
        ...
        Promise.all(
            subcategories.map(async (sub) => {
                try {
                    const res = await apiService.getProducts({ subcategory: sub._id, limit: 1, status: 'active' });
                    return { id: sub._id, has: (res.data?.length ?? 0) > 0 };
                } catch {
                    return { id: sub._id, has: false };
                }
            })
        ).then((results) => {
            if (cancelled) return;
            results.forEach((r) => { map[r.id] = r.has; });
            setSubcategoryHasProducts(map);
        });
        return () => { cancelled = true; };
    }, [subcategories]);
    ...
    const renderCategoryItem = ({ item }: { item: Category }) => {
        ...
        const hasProducts = categoryHasProducts[item._id] !== false;
        return (
            <TouchableOpacity
                style={[styles.categoryCard, !hasProducts && styles.categoryCardDisabled]}
                onPress={() => hasProducts && handleCategoryPress(item)}
                activeOpacity={0.8}
                disabled={!hasProducts}
            >
                <View style={[styles.categoryIconContainer, !hasProducts && styles.categoryIconDisabled]}>
                    {imageUri ? (
                        <Image 
                            source={{ uri: imageUri }} 
                        ...
                        <Ionicons name="grid-outline" size={32} color={hasProducts ? '#007AFF' : '#ccc'} />
                    )}
                </View>
                <View style={styles.categoryInfo}>
                    <Text style={[styles.categoryName, !hasProducts && styles.categoryTextDisabled]} numberOfLines={2}>
                        {item.name}
                    </Text>
                </View>
    ...
    const renderSubcategoryItem = ({ item }: { item: Category }) => {
        const imageUri = item.image || undefined;
        const hasProducts = subcategoryHasProducts[item._id] !== false;
        return (
            <TouchableOpacity
                style={[styles.subcategoryCard, !hasProducts && styles.subcategoryCardDisabled]}
                onPress={() => hasProducts && handleSubcategoryPress(item)}
                activeOpacity={0.8}
                disabled={!hasProducts}
            >
                <View style={[styles.subcategoryIconContainer, !hasProducts && styles.subcategoryIconDisabled]}>
                    {imageUri ? (
                        <Image 
                            source={{ uri: imageUri }} 
                        ...
                        <Ionicons name="folder-outline" size={28} color={hasProducts ? '#007AFF' : '#ccc'} />
                    )}
                </View>
                <View style={styles.subcategoryInfo}>
                    <Text style={[styles.subcategoryName, !hasProducts && styles.subcategoryTextDisabled]} numberOfLines={2}>
                        {item.name}
                    </Text>
                </View>
                <Ionicons name="chevron-forward" size={24} color="#ccc" />
