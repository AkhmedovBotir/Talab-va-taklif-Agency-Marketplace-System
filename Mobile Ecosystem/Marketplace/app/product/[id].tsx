...
import QuillDeltaRenderer from '../../components/ui/QuillDeltaRenderer';
...
export default function ProductDetailScreen() {
    ...
    const loadProduct = async () => {
        ...
        try {
            ...
            let isMaxalla = false;
            try {
                response = await apiService.getProductById(productId);
            } catch (error: any) {
                if (error.status === 404 || error.message?.includes('topilmadi')) {
                    response = await apiService.getMaxallaProductById(productId);
                    isMaxalla = true;
                } else {
                    throw error;
                }
