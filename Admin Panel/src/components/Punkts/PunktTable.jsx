...
const PunktTable = ({ punkts, loading, onEdit, onDelete, onView, pagination, onPageChange, onStatusChange }) => {
  ...
  const handleStatusToggle = async (punkt, newStatus) => {
    ...
    try {
      const response = await punktAPI.updatePunkt(punkt._id, { status: newStatus });
      if (response.success) {
      ...
      setUpdatingStatus({ ...updatingStatus, [punkt._id]: false });
    }
  };
  ...
  if (punkts.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <p className="text-center text-gray-500">Punktlar topilmadi</p>
      </div>
    );
  }
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tuman</th>
              ...
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={punkt.status === 'active'}
                      onChange={(e) => {
                        const newStatus = e.target.checked ? 'active' : 'inactive';
                        handleStatusToggle(punkt, newStatus);
                      }}
                      disabled={updatingStatus[punkt._id]}
