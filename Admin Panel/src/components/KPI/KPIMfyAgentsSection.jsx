...
const KPIMfyAgentsSection = () => {
  ...
  const fetchAgents = async ({ page, limit } = {}) => {
    ...
    try {
      ...
      const response = await kpiAPI.getMfyAgentsKPI(params);
      if (response.success) {
      ...
      showError(error.message || 'MFY agentlarini yuklashda xatolik');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResetFilters = () => {
    setFilters({ viloyatId: '', tumanId: '', mfyId: '', isPaid: '', startDate: '', endDate: '' });
    setTumanlar([]);
    setMfylar([]);
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">MFY agentlari topilmadi</div>
      ) : (
        <>
...
export default KPIMfyAgentsSection;
