...
const CreateBaseProductModal = ({ open, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await baseProductAPI.createBaseProduct(payload);

      if (response.success) {
        showSuccess(response.message || 'Shablon muvaffaqiyatli yaratildi');
        onSuccess();
        onClose();
      }
    } catch (err) {
      const errorMsg = err.message || 'Shablon yaratishda xatolik yuz berdi';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
            ...
            >
              ...
              <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-700 text-white p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-lg font-bold">Yangi Shablon</h2>
                    <p className="text-xs text-indigo-100 mt-0.5">
                      Asosiy maxsulot shablonini yarating
                    </p>
                  </div>
                  <button
                    onClick={handleClose}
