...
const DeleteCategoryModal = ({ open, onClose, onSuccess, category, isSubcategory = false }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      let response;
      if (isSubcategory) {
        response = await categoryManagementAPI.deleteSubcategory(category._id);
      } else {
        response = await categoryManagementAPI.deleteCategory(category._id);
      }

      if (response.success) {
        showSuccess(response.message || `${isSubcategory ? 'Subkategoriya' : 'Kategoriya'} muvaffaqiyatli o'chirildi`);
        onSuccess();
        onClose();
      }
    } catch (err) {
      const errorMsg = err.message || `${isSubcategory ? 'Subkategoriya' : 'Kategoriya'} o'chirishda xatolik yuz berdi`;
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-bold">
                        {isSubcategory ? 'Subkategoriyani o\'chirish' : 'Kategoriyani o\'chirish'}
                      </h2>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-gray-900 font-medium mb-2">
                      Siz haqiqatan ham <span className="font-bold text-red-600">"{category.name}"</span>{' '}
                      {isSubcategory ? 'subkategoriyasini' : 'kategoriyasini'} o'chirmoqchimisiz?
                    </p>
                    {!isSubcategory && (
                      <p className="text-sm text-gray-600 mb-2">
                        Agar kategoriyada subkategoriyalar bo'lsa, ularni avval o'chirishingiz kerak.
                      </p>
                    )}
