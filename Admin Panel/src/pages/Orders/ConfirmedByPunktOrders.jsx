...
const ConfirmedByPunktOrders = ({ hideHeader = false }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await adminDataAPI.getOrdersConfirmedByPunkt(params);
      ...
      const errorMsg = err.message || 'Punkt qabul qilgan buyurtmalarni yuklashda xatolik yuz berdi';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      {!hideHeader && (
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Punkt Qabul Qilgan Buyurtmalar</h1>
            <p className="text-gray-600">Punkt tomonidan qabul qilingan, lekin hali keyingi bosqichga o'tmagan buyurtmalar</p>
          </div>
        </motion.div>
      )}
...
export default ConfirmedByPunktOrders;
