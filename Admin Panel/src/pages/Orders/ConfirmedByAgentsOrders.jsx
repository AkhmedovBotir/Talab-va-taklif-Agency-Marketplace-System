...
const ConfirmedByAgentsOrders = ({ hideHeader = false }) => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await adminDataAPI.getOrdersConfirmedByAgents(params);
      ...
      const errorMsg = err.message || 'Agent topshirgan buyurtmalarni yuklashda xatolik yuz berdi';
      setError(errorMsg);
      showError(errorMsg);
    } finally {
      setLoading(false);
  ...
  return (
    <div>
      {!hideHeader && (
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Agent Topshirgan Buyurtmalar</h1>
            <p className="text-gray-600">Agentlar tomonidan mijozlarga yetkazilgan va tasdiqlangan buyurtmalar</p>
          </div>
        </motion.div>
      )}
...
export default ConfirmedByAgentsOrders;
