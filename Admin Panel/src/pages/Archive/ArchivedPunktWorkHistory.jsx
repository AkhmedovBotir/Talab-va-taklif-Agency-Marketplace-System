...
const ArchivedPunktWorkHistory = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchWorkHistory = async () => {
      setLoading(true);
      try {
        const response = await archiveAPI.getArchivedPunktWorkHistory(id);
        if (response.success) {
          setPunkt(response.punkt);
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800">Punkt topilmadi</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{punkt.name}</h1>
      ...
      >
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Punkt ma'lumotlari</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p className="text-sm text-gray-500">Telefon</p>
            <p className="text-base font-medium text-gray-900">{punkt.phone || '-'}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Viloyat</p>
            <p className="text-base font-medium text-gray-900">{punkt.viloyat?.name || '-'}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Tuman</p>
            <p className="text-base font-medium text-gray-900">{punkt.tuman?.name || '-'}</p>
          </div>
          <div>
