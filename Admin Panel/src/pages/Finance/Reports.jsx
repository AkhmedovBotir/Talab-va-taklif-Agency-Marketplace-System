  ...
  const renderDailyReport = () => {
    ...
    return (
      <>
        ...
        {dailyReport.submissions && dailyReport.submissions.length > 0 && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-white rounded-lg shadow-sm border border-gray-200 p-6"
          >
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Topshiruvlar</h2>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Summa</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Transaksiyalar</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {dailyReport.submissions.map((submission, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-900">{submission.fromAgent?.name || '-'}</td>
                      <td className="px-4 py-3 text-sm text-right text-gray-900">{formatAmount(submission.amount)}</td>
                      <td className="px-4 py-3 text-sm text-right text-gray-500">{submission.transactionsCount}</td>
                      <td className="px-4 py-3 text-sm">
                        ...
                        >
                          {submission.status === 'confirmed' ? 'Tasdiqlangan' : submission.status === 'rejected' ? 'Rad etilgan' : 'Kutilmoqda'}
                        </span>
                      </td>
                    </tr>
                  ))}
