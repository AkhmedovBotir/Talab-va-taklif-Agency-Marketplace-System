...
Punktlar uchun notification inbox:
- faqat `target_type = punkts` va `target_type = all` yozuvlar ko'rinadi
...
Auth: `Authorization: Bearer <punkt_jwt>` (punkt login tokeni)

Barcha marshrutlar `/punkts/me` ostida — ya'ni punkt autentifikatsiyasi talab qilinadi.

## 1) Ro'yxat

`GET /punkts/me/notifications?page=1&limit=10`
...
### Nishon (`target_type`) — punkt uchun

Punkt inboxda faqat:
- `all` — hammaga yuborilgan xabarlar
- `punkts` — faqat punktlarga yuborilgan xabarlar

Boshqa nishonlar (`admins`, `agents`, `managers`, …) punkt API da ko'rinmaydi.

## 2) O'qilmaganlar soni (badge)

`GET /punkts/me/notifications/unread-count`
...
`PATCH /punkts/me/notifications/:id/read`

- `200` — `"Notification o'qildi deb belgilandi"`
- `404` — topilmadi yoki punkt uchun ko'rinmaydi

## 4) Hammasini o'qildi qilish

`PATCH /punkts/me/notifications/read-all`

- `200` — `"Barcha notificationlar o'qildi deb belgilandi"`

## 5) Real-time Socket

`GET /punkts/me/notifications/ws`

Auth:
- Browser WebSocket uchun query token:
  - `ws://localhost:8081/api/v1/punkts/me/notifications/ws?token=<punkt_jwt>`
...
Eslatma: `target_type=punkts` va `target_type=all` eventlar punkt socketiga yetkaziladi.
