...
type ManagerOrderPipelineItem struct {
	ID                                   uint                            `json:"id"`
	...
	Items                                []ManagerOrderPipelineOrderItem `json:"items"`
	CurrentStage                         string                          `json:"current_stage"`
	CreatedAt                            string                          `json:"created_at"`
	UpdatedAt                            string                          `json:"updated_at"`
}

type ManagerOrderPipelineOrderItem struct {
	ID             uint    `json:"id"`
...
type ManagerPaginatedPipelineItems struct {
	Items      []ManagerOrderPipelineItem `json:"items"`
...
type ManagerPipelineOverview struct {
...
type ManagerOrderPipelineService interface {
	ListByStage(regionID uint, stage string, page, limit int) (*ManagerPaginatedPipelineItems, error)
	Overview(regionID uint) (*ManagerPipelineOverview, error)
}

type managerOrderPipelineService struct {
	repo repository.ManagerOrderPipelineRepository
	db   *gorm.DB
}

func NewManagerOrderPipelineService(repo repository.ManagerOrderPipelineRepository, db *gorm.DB) ManagerOrderPipelineService {
	return &managerOrderPipelineService{repo: repo, db: db}
}

func (s *managerOrderPipelineService) ListByStage(regionID uint, stage string, page, limit int) (*ManagerPaginatedPipelineItems, error) {
	rows, total, err := s.repo.ListByStage(regionID, stage, page, limit)
	if err != nil {
		return nil, err
	}
	...
	items := make([]ManagerOrderPipelineItem, 0, len(rows))
	for i := range rows {
		r := &rows[i]
		...
		itemRows := make([]ManagerOrderPipelineOrderItem, 0, len(r.Items))
		for j := range r.Items {
			it := &r.Items[j]
			itemRows = append(itemRows, ManagerOrderPipelineOrderItem{
				ID:             it.ID,
		...
		items = append(items, ManagerOrderPipelineItem{
			ID:                                   r.ID,
			Status:                               r.Status,
			...
			Items:                                itemRows,
			CurrentStage:                         detectManagerCurrentStage(r),
	...
	return &ManagerPaginatedPipelineItems{
		Items:      items,
		Total:      total,
		Page:       page,
		Limit:      limit,
...
func (s *managerOrderPipelineService) Overview(regionID uint) (*ManagerPipelineOverview, error) {
	get := func(stage string) (int64, error) { return s.repo.CountByStage(regionID, stage) }
	out := &ManagerPipelineOverview{}
	var err error
...
func (s *managerOrderPipelineService) resolveRelatedNames(rows []mpdomain.Order) (map[uint]string, map[uint]string, map[uint]string, error) {
	...
	if len(contragentIDs) > 0 {
		var contragents []adminDomain.Contragent
...
func detectManagerCurrentStage(r *mpdomain.Order) string {
