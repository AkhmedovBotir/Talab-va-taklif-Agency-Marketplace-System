...
export default function SendCodeScreen() {
  ...
  const sendCode = async () => {
    ...
    setError('');
    try {
      await apiService.registerSendCode(phone);
      setCountdown(60);
      Alert.alert('Muvaffaqiyatli', 'Tasdiqlash kodi yuborildi');
    } catch (err: any) {
  ...
  const handleCodeComplete = async (completeCode: string) => {
    ...
    setLoading(true);
    try {
      await apiService.registerVerifyCode(phone!, completeCode);
      router.push({
        pathname: '/auth/register/form',
        params: { phone: phone!, code: completeCode },
      });
    } catch (err: any) {
  ...
  const handleResend = async () => {
    ...
    setError('');
    try {
      await apiService.resendCode(phone!, 'register');
      setCountdown(60);
