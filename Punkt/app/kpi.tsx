...
export default function KPIScreen() {
  ...
  /** Tor mobil: vertikal; web: 3 ustun (RN Web vertikal stackda matn qoplashi mumkin) */
  const stackStats = !isWeb && windowWidth < 520;
  ...
  const ListHeader = useMemo(() => {
    const rowStyle = stackStats ? styles.kpiRowStack : styles.kpiRow;
    const showSummaryCard = sortedDays.length > 0;

    const statsRowStyle = [rowStyle, isWeb && !stackStats && styles.kpiRowWeb];
    const statsItemStyle = [
      styles.kpiItem,
      stackStats && styles.kpiItemStack,
      isWeb && !stackStats && styles.kpiItemWeb,
    ];
    const statsLabelStack = stackStats ? styles.kpiLabelStack : undefined;
    const statsValueStack = stackStats ? styles.kpiValueStack : undefined;
        ...
        {today ? (
          <View style={[styles.kpiCard, isWeb && styles.kpiCardWeb]}>
            ...
            <View style={statsRowStyle}>
              <View style={statsItemStyle}>
                <Text style={[styles.kpiLabel, statsLabelStack]}>Punkt KPI (jami)</Text>
                <Text style={[styles.kpiValue, statsValueStack]}>
                  {today.punkt_kpi_total.toLocaleString('uz-UZ')} so‘m
                </Text>
              </View>
              {stackStats ? <View style={styles.kpiDividerH} /> : <View style={styles.kpiDivider} />}
              <View style={statsItemStyle}>
                <Text style={[styles.kpiLabel, statsLabelStack]}>To‘langan</Text>
                <Text style={[styles.kpiValue, styles.kpiPaid, statsValueStack]}>
                  {today.paid_total_today.toLocaleString('uz-UZ')} so‘m
                </Text>
              </View>
              {stackStats ? <View style={styles.kpiDividerH} /> : <View style={styles.kpiDivider} />}
              <View style={statsItemStyle}>
                <Text style={[styles.kpiLabel, statsLabelStack]}>To‘lanmagan</Text>
                <Text style={[styles.kpiValue, styles.kpiUnpaid, statsValueStack]}>
  ...
  }, [today, sortedDays.length, stackStats, isWeb, appliedFrom, appliedTo, rangeTotals]);
