...
export default function ProductComments() {
  ...
  const fetchComments = async () => {
    try {
      ...
      if (!selectedId && list.length > 0) {
        setSelectedId(list[0].id || list[0].rating_id);
      }
    } catch (err) {
  ...
  const fetchCommentDetail = async (ratingId) => {
    try {
      ...
      setSelectedCase(response?.data || response || null);
    } catch (err) {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">Escalated</label>
              <select
          ...
          <div className="xl:col-span-1 bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="max-h-[520px] overflow-y-auto divide-y divide-gray-100">
                {rows.map((item) => {
                  const id = item.id || item.rating_id;
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-sm text-gray-600 mt-1 line-clamp-2">
                        {item.comment || item.note || item.text || "Komment matni yo'q"}
                      </p>
                    </button>
                  );
                })}
            ...
            {detailLoading ? (
              ...
              <div className="space-y-5">
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-lg font-semibold text-gray-900">Case #{selectedCase.id || selectedId}</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-900">{selectedCase.product_name || selectedCase.product?.name || '-'}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-xs text-gray-500 mb-1">Kontragent</p>
                    <p className="text-sm text-gray-900">{selectedCase.contragent_name || selectedCase.contragent?.name || '-'}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-xs text-gray-500 mb-1">Foydalanuvchi</p>
                    <p className="text-sm text-gray-900">{selectedCase.user_name || selectedCase.user?.name || selectedCase.user_id || '-'}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-xs text-gray-500 mb-1">Baho</p>
                    <p className="text-sm text-gray-900">{selectedCase.rating || '-'}</p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-sm text-gray-900 whitespace-pre-wrap">
                    {selectedCase.comment || selectedCase.text || "Komment matni yo'q"}
                  </p>
                </div>

                <div>
                  <label className="block text-sm text-gray-700 mb-2">Izoh (note/call/escalate/resolve uchun)</label>
                  <textarea
                    value={note}
