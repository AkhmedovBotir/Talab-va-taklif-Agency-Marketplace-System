...
const OrderPipelineMonitor = () => {
  ...
  const loadRelatedDetails = useCallback(
    async (orders) => {
      ...
      const missingUserIds = uniqueUserIds.filter((id) => !relatedData.users[toKey(id)]);
      const missingPunktIds = uniquePunktIds.filter((id) => !relatedData.punkts[toKey(id)]);
      const missingAgentIds = uniqueAgentIds.filter((id) => !relatedData.agents[toKey(id)]);
      const missingContragentIds = uniqueContragentIds.filter((id) => !relatedData.contragents[toKey(id)]);
    ...
    [relatedData.users, relatedData.punkts, relatedData.agents, relatedData.contragents]
  );
            ...
            <tbody>
              {tableLoading ? (
                ...
                filteredItems.map((item) => (
                  <tr key={item.id ?? item._id} className="border-t border-gray-100 hover:bg-gray-50">
