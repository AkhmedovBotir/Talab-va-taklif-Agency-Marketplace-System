...
const NeighborhoodShopTable = ({
  ...
  const handleStatusToggle = async (row, status) => {
    ...
    try {
      const res = await neighborhoodShopAPI.updateStatus(id, status);
      if (res.success) {
  ...
  if (!rows.length) {
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
        <p className="text-center text-gray-500">Maxalla do‘konlari topilmadi</p>
      </div>
    );
  }
            ...
            {rows.map((row, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">{row.inn?.trim() ? row.inn : '-'}</td>
...
export default NeighborhoodShopTable;
