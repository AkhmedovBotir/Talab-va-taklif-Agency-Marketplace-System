...
function getEntityId(entity: { id?: number; _id?: string }): string {
...
export default function FinanceScreen() {
  ...
  const loadTransactions = useCallback(async (params?: GetPaymentTransactionsParams) => {
    try {
      ...
      showSnackbar(error.response?.data?.message || 'Tranzaksiyalarni yuklashda xatolik', { variant: 'error' });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const loadOrdersForPayment = useCallback(async () => {
    try {
      ...
      showSnackbar(
        error.response?.data?.message || 'To\'lov qilish kerak bo\'lgan buyurtmalarni yuklashda xatolik',
        { variant: 'error' }
      );
    } finally {
  ...
  const handlePayToPunkt = async (orderId: string) => {
    const order = ordersForPayment.find((o) => getEntityId(o) === orderId);
    if (!order) return;

    const ok = await showConfirm({
      title: 'Punktga to\'lov qilish',
      message: `Haqiqatan ham punktga ${(order.totalPrice || 0).toLocaleString()} so'm to'lov qilmoqchimisiz?`,
      confirmText: 'To\'lov qilish',
    ...
    try {
      ...
      if (response.success) {
        showSnackbar(response.message || 'To\'lov muvaffaqiyatli amalga oshirildi', { variant: 'success' });
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      const detailId = getEntityId(item);
                      if (!detailId) {
                        showSnackbar("Buyurtma ID topilmadi", { variant: 'error' });
                        return;
                      }
                      router.push(`/order/${detailId}`);
                    }}
                  >
                    ...
                    <TouchableOpacity
                      style={[styles.payButton, payingOrderId === getEntityId(item) && styles.payButtonDisabled]}
                      onPress={() => {
                        const payId = getEntityId(item);
                        if (!payId) {
                          showSnackbar("Buyurtma ID topilmadi", { variant: 'error' });
                          return;
                        }
                        handlePayToPunkt(payId);
                      }}
                      disabled={payingOrderId === getEntityId(item)}
                    >
                      {payingOrderId === getEntityId(item) ? (
