...
/** Eski token yaroqsiz bo‘lsa — sessiyani tozalash, login oynasini ochmaslik. */
let sessionInvalidatedHandler: AuthHandler | null = null;
/** Foydalanuvchi savat / profil / checkout bosganda — login ko‘rsatish. */
let authLoginPromptHandler: AuthHandler | null = null;

export function setSessionInvalidatedHandler(handler: AuthHandler | null) {
  sessionInvalidatedHandler = handler;
}

export function setAuthLoginPromptHandler(handler: AuthHandler | null) {
  authLoginPromptHandler = handler;
}

/** @deprecated — `setSessionInvalidatedHandler` ishlating */
export function setAuthFailureHandler(handler: AuthHandler | null) {
  setSessionInvalidatedHandler(handler);
}

/** Faqat foydalanuvchi harakati bilan (savatga qo‘shish, korzinka tab, …). */
...
async function request<T>(path: string, method: HttpMethod = 'GET', body?: unknown, auth = false): Promise<T> {
  ...
  if (auth) {
    ...
    if (!token) {
      throw new Error("Savat uchun kirish kerak. Iltimos, avval tizimga kiring.");
    }
...
export const api = {
  ...
  localShops: {
    list: async (params?: {
      ...
      district_id?: number;
      mfy_id?: number;
    }): Promise<{ items: LocalShopPublic[]; total: number; page: number; limit: number; total_pages: number }> => {
      const page = params?.page ?? 1;
      ...
      const qs = [
        ...
        .join('&');
      try {
        const raw = await request<any>(`/noauth/local-shops?${qs}`);
        const data = unwrapData<any>(raw);
        const items = (Array.isArray(data?.items) ? data.items : extractArray<any>(data)).map((row: any): LocalShopPublic => ({
          id: Number(row?.id ?? 0),
          name: String(row?.name ?? ''),
          region_id: row?.region_id != null ? Number(row.region_id) : undefined,
          district_id: row?.district_id != null ? Number(row.district_id) : undefined,
          mfy_id: row?.mfy_id != null ? Number(row.mfy_id) : undefined,
          phone: row?.phone != null ? String(row.phone) : undefined,
          logo: row?.logo != null ? String(row.logo) : undefined,
          status: row?.status != null ? String(row.status) : undefined,
          working_hours: normalizeLocalShopWorkingHours(row?.working_hours),
        }));
        return {
          items,
    ...
    getWorkingHoursMap: async (shopIds: number[]): Promise<Map<number, LocalShopWorkingHour[]>> => {
      const wanted = new Set(shopIds.filter((id) => Number.isFinite(id) && id > 0));
      const map = new Map<number, LocalShopWorkingHour[]>();
      if (!wanted.size) return map;

      let page = 1;
      let totalPages = 1;
      while (page <= totalPages && page <= 10) {
        const res = await api.localShops.list({ page, limit: 100 });
        totalPages = Math.max(1, res.total_pages);
        for (const shop of res.items) {
          if (wanted.has(shop.id) && shop.working_hours?.length) {
            map.set(shop.id, shop.working_hours);
          }
        }
