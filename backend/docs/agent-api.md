# Agent API

Base URL: `http://localhost:8081/api/v1`

Modul fayllari: `modules/admin` ichida `agent_` prefiks (`agent.go`, `agent_repository.go`, `agent_service.go`, `agent_http.go`).

## Muhim

- Barcha endpointlar `Authorization: Bearer <token>` talab qiladi.
- Faqat `general` role ushbu endpointlardan foydalana oladi.
- **Yaratishda** parol bazaga yozilmaydi (`password` e'tiborsiz). `password_setup_allowed` (default `true`) keyinroq parol qo‘yishga ruxsat beradi.
- **Tahrirlashda** `password` ixtiyoriy yuborilsa — **bcrypt** bilan hashlanib saqlanadi; bo‘sh qoldirilsa parol o‘zgarmaydi.
- Login hozircha **alohida endpoint sifatida yo‘q**; javobda `has_password` bor.

## Maydonlar (JSON)

| Maydon | Tavsif |
|--------|--------|
| `name` | To‘liq nom |
| `viloyat_id` | Viloyat (`regions` jadvalidagi `id`) |
| `tuman_id` | Tuman (`districts` jadvalidagi `id`) |
| `mfy_id` | MFY (`mfies` jadvalidagi `id`) |
| `phone` | `+998901234567` formatida, **unique** (`agents` ichida) |
| `status` | `active` yoki `inactive` |
| `password_setup_allowed` | Ixtiyoriy, default `true` |
| `password` | Faqat **PUT** da ixtiyoriy |

Viloyat–tuman–MFY mosligi tekshiriladi (tuman shu viloyatga, MFY shu tumanga tegishli bo‘lishi kerak).

## 1) Yaratish

**POST** `/agents`

```json
{
  "name": "Ali Valiyev",
  "viloyat_id": 1,
  "tuman_id": 1,
  "mfy_id": 1,
  "phone": "+998901112233",
  "status": "active",
  "password_setup_allowed": true
}
```

## 2) Ro‘yxat (pagination)

**GET** `/agents?page=1&limit=10`

- `page` — default `1`
- `limit` — default `10`, max `100`

## 3) Bitta agent

**GET** `/agents/{id}`

## 4) Yangilash

**PUT** `/agents/{id}`

```json
{
  "name": "Ali Valiyev",
  "viloyat_id": 1,
  "tuman_id": 1,
  "mfy_id": 1,
  "phone": "+998901112233",
  "status": "active",
  "password_setup_allowed": false,
  "password": "yangiParol123"
}
```

## 5) Status

**PATCH** `/agents/{id}/status`

```json
{
  "status": "inactive"
}
```

## 6) O‘chirish

**DELETE** `/agents/{id}`

## 7) Status kodlar

- `200` — OK
- `201` — Yaratildi
- `400` — Validatsiya
- `401` / `403` — Auth / general emas
- `404` — Topilmadi
- `409` — Telefon band
- `500` — Server xatosi
