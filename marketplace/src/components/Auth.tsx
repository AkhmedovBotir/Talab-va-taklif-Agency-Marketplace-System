      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <View className="flex-1 flex-row">
          ...
          <View className="flex-1 items-center justify-center bg-gray-50/50 px-4 py-6">
            <View className="w-full max-w-md rounded-[40px] border border-gray-100 bg-white p-6 shadow-xl md:p-8">
              <View className="mb-7">
                  ...
                  <View className="gap-2 my-3">
                    ...
                    <View className="relative">
                      ...
                      <TextInput
                        ...
                        className="w-full rounded-2xl border border-gray-200 bg-gray-50 py-4 pl-14 pr-4 text-lg font-bold text-gray-900"
                  ...
                  <View className="my-1 flex-row justify-between gap-2">
                    {code.map((digit, i) => (
                      <TextInput
                        ...
                        className="h-16 flex-1 rounded-2xl border-2 border-gray-200 bg-gray-50 text-center text-2xl font-black text-gray-900"
                        style={
                          Platform.OS === 'web'
                            ? { width: 56, minWidth: 56, maxWidth: 56, flexGrow: 0, flexShrink: 0, marginVertical: 4 }
                            : undefined
                        }
                      />
                    ))}
              ...
              {step === 'register' ? (
                ...
                >
                  <View className="my-1 flex-row" style={{ columnGap: 12 }}>
                    <TextInput
                      value={regData.first_name}
                      onChangeText={(val) => setRegData((p) => ({ ...p, first_name: val }))}
                      placeholder="Ism"
                      className="rounded-xl border border-gray-200 bg-gray-50 p-3.5 font-bold text-gray-900"
                  ...
                  {Platform.OS === 'web' ? (
                    <View className="w-full overflow-hidden rounded-xl border border-gray-200 bg-gray-50">
                      <input
                        type="date"
                        ...
                        className="box-border min-h-[48px] w-full cursor-pointer border-0 bg-transparent px-3.5 py-3 text-base font-bold text-gray-900 outline-none"
                        style={{ margin: '4px 0' }}
                      />
                    </View>
                  ) : (
                    <>
                      ...
                      >
                        <Text className={cn('text-base font-bold', birthDateIsoValue ? 'text-gray-900' : 'text-gray-400')}>
                          {birthDateIsoValue
                  ...
                  <Pressable onPress={() => setSelector('region')} className="w-full flex-row items-center justify-between rounded-xl border border-gray-200 bg-gray-50 p-4">
                    <Text className="font-bold text-gray-900">{regionsList.find((r) => r.id === regData.region_id)?.name || 'Viloyatni tanlang'}</Text>
                    <ChevronRight size={18} color="#9ca3af" />
                  </Pressable>
                  <Pressable
                    onPress={() => setSelector('district')}
                    disabled={!regData.region_id}
                    className="w-full flex-row items-center justify-between rounded-xl border border-gray-200 bg-gray-50 p-4 disabled:opacity-50"
                  >
                  ...
                  <Pressable
                    ...
                    className="w-full flex-row items-center justify-between rounded-xl border border-gray-200 bg-gray-50 p-4 disabled:opacity-50"
                  >
                    <Text className="font-bold text-gray-900">{mfysList.find((m) => m.id === regData.mfy_id)?.name || 'MFY ni tanlang'}</Text>
                    <ChevronRight size={18} color="#9ca3af" />
                  </Pressable>

                  <Pressable onPress={onFinishRegister} disabled={loading} className="mt-4 mb-1 items-center rounded-2xl bg-gray-900 py-5">
          ...
          >
            ...
            <View className="mb-3">
              <TextInput
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-3 font-semibold text-slate-900"
              />
            </View>
            <ScrollView>
