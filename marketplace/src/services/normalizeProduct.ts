...
export function normalizeLocalShopProduct(row: any): LocalShopProduct {
  ...
  return {
    ...
    local_shop: row?.shop || row?.local_shop
      ? {
          id: Number((row.shop ?? row.local_shop).id ?? 0),
          name: String((row.shop ?? row.local_shop).name ?? ''),
          region_id: (row.shop ?? row.local_shop).region_id != null ? Number((row.shop ?? row.local_shop).region_id) : undefined,
          district_id: (row.shop ?? row.local_shop).district_id != null ? Number((row.shop ?? row.local_shop).district_id) : undefined,
          mfy_id: (row.shop ?? row.local_shop).mfy_id != null ? Number((row.shop ?? row.local_shop).mfy_id) : undefined,
          phone: (row.shop ?? row.local_shop).phone != null ? String((row.shop ?? row.local_shop).phone) : undefined,
        }
      : undefined,
