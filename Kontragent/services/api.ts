...
import { notifyAuthUnauthorized } from './authSessionEvents';
...
class ApiService {
  ...
  private throwApiError(
    ...
    options?: { hadAuth?: boolean }
  ): never {
    const status = response.status;
    if (options?.hadAuth && (status === 401 || status === 403)) {
      const serverMsg = typeof data.message === 'string' ? data.message.trim() : '';
      notifyAuthUnauthorized(
        serverMsg ||
          (status === 401 ? 'Sessiya tugadi. Qaytadan kiring.' : 'Kirish huquqi yo\'q')
      );
    }
    throw {
      status,
