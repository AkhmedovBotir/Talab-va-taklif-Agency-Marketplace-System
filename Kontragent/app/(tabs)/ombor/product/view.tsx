...
function ProductDescriptionWeb({ delta }: { delta: DeltaFormat }) {
  const containerRef = useRef<any>(null);
  const idRef = useRef('product-desc-' + Math.random().toString(36).slice(2));
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const quill = new Quill('#' + idRef.current, {
          theme: 'snow',
          readOnly: true,
        ...
        console.error('Quill description init error:', e);
      }
    })();
    return () => {
      mounted = false;
  ...
  return <View ref={containerRef} style={styles.descriptionWebView} collapsable={false} />;
}
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        <TouchableOpacity
          onPress={() => router.push('/ombor' as any)}
        ...
        {product.description && (
          <View style={styles.card}>
            ...
            <View style={styles.descriptionContainer}>
              {Platform.OS === 'web' ? (
                <ProductDescriptionWeb delta={product.description} />
              ) : (
                <WebView
                  originWhitelist={['*']}
                  source={{
                    ...
                    <html>
                      <head>
                        ...
                        <style>
                          ...
                          .ql-editor { padding: 0; font-size: 15px; line-height: 22px; color: #666; }
                      ...
                      <body>
                        ...
                        <script>
                          var quill = new Quill('#editor-container', { theme: 'snow', readOnly: true, modules: { toolbar: false } });
                          quill.setContents(${JSON.stringify(product.description)});
                        </script>
                      </body>
                    </html>
                  `,
