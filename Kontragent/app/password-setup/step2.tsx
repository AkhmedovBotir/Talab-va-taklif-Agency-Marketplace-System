...
/** Veb TextInput: shrinadi, default outline yo'q */
const webCodeInputFix =
  Platform.OS === 'web'
    ? ({
        outlineStyle: 'none',
...
export default function PasswordSetupStep2Screen() {
  ...
  /** Kartaning ichki kengligi (padding + maxWidth) — katak o‘lchami vebda shrinadi */
  const contentRowWidth = Math.min(400, Math.max(0, windowWidth - 40));
  const otpRowInnerWidth = contentRowWidth - 48;
  const otpGap = 8;
  const otpCellSide = Math.max(
    40,
    Math.min(56, Math.floor((otpRowInnerWidth - otpGap * 4) / 5))
  );
  ...
  const handleResendCode = async () => {
    setResending(true);
    try {
      await apiService.passwordSetupResendCode({ phone });
      showSnackbar('Yangi tasdiqlash kodi telefon raqamingizga yuborildi', {
  ...
  const handleVerifyCode = async (codeValue?: string) => {
    ...
    try {
      ...
      showSnackbar('Kod muvaffaqiyatli tasdiqlandi. Endi parol o\'rnatishingiz mumkin', {
        title: 'Muvaffaqiyatli',
        variant: 'success',
      });
      router.push({
      ...
      if (error.status === 400) {
        ...
        errorMessage = error.message || 'SMS kod muddati o\'tgan';
      } else if (error.message) {
        errorMessage = error.message;
      }
          ...
          <View style={styles.card}>
            <View style={styles.form}>
              ...
              <View style={styles.codeContainer}>
                ...
                <View style={[styles.codeInputsWrapper, { gap: otpGap }]}>
                  {code.map((digit, index) => (
                    <View key={index} style={[styles.codeInputCell, { width: otpCellSide, height: otpCellSide }]}>
                      <TextInput
                        ref={(ref) => {
                        ...
                        style={[
                          styles.codeInput,
                          { fontSize: otpCellSide >= 48 ? 26 : 20 },
                          digit !== '' && styles.codeInputFilled,
                          Platform.OS === 'web' && webCodeInputFix,
                        ]}
                        value={digit}
